...
bool AntiCheat::validateClaim(Entity& p, double& px, double& pz, uint64_t nowMs,
  ...
  // 检查 B：地形校验（防穿墙）——中心点判定
  // 浮岛世界设定：无建筑墙壁，允许模型一半悬空，双端统一为中心点判定（客户端
  // predict.js 的 circleBlocked 同样只查圆心）。此处核实圆心（上报点）不落入不可
  // 通行格（湖泊/河流/悬崖/陡坡），防瞬移进阻挡区/作弊直报阻挡区内点。中心点边界处
  // 的浮点/量化分歧沿线段夹回，软失败不计违规。相比旧实现（圆盘 8 点采样 + 半径
  // 收缩容差 + 夹紧到圆盘边界），不再校验半径——悬空/贴边由设定允许，圆心可通行即通过。
