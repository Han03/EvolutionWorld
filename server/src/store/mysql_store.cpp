...
bool MysqlStore::savePlayer(const PlayerSave& s) {
  ...
  MYSQL_STMT* st = mysql_stmt_init(impl_->conn);
  if (!st) return false;
  const char* sql = "INSERT INTO player_saves(username,x,y,z,hp,level,updated_at,gold,equip_json,inventory_json,exp) "
                    "VALUES(?,?,?,?,?,?,?,?,?,?,?) "
                    "ON DUPLICATE KEY UPDATE x=VALUES(x),y=VALUES(y),z=VALUES(z),hp=VALUES(hp),level=VALUES(level),"
                    "updated_at=VALUES(updated_at),gold=VALUES(gold),equip_json=VALUES(equip_json),inventory_json=VALUES(inventory_json),exp=VALUES(exp)";
  bool ok = false;
  if (mysql_stmt_prepare(st, sql, (unsigned long)strlen(sql)) == 0) {
    MYSQL_BIND b[11]; memset(b,0,sizeof(b));
...
bool MysqlStore::loadPlayer(const std::string& username, PlayerSave& out) {
  ...
  MYSQL_STMT* st = mysql_stmt_init(impl_->conn);
  if (!st) return false;
  const char* sql = "SELECT username,x,y,z,hp,level,updated_at,gold,equip_json,inventory_json,exp FROM player_saves WHERE username=?";
  bool found = false;
  if (mysql_stmt_prepare(st, sql, (unsigned long)strlen(sql)) == 0) {
    ...
    if (mysql_stmt_bind_param(st, &pb) == 0 && mysql_stmt_execute(st) == 0) {
      ...
      MYSQL_BIND rb[11]; memset(rb,0,sizeof(rb));
      rb[0].buffer_type=MYSQL_TYPE_STRING; rb[0].buffer=name; rb[0].buffer_length=sizeof(name); rb[0].length=&nameLen;
